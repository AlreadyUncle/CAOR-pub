...
{ float delta_x = vecx[i] - x;
  ...
  if (fabs(delta_theta) > check_angle){
    if (fabs(delta_theta) <= M_PI){
      if (delta_theta>=0){
        float R = dist/(2*sin(delta_theta));
        updatecommandVelocity(LINEAR_VELOCITY, LINEAR_VELOCITY/R);
      }
      else{
        float R = dist/(2*sin(fabs(delta_theta)));
        updatecommandVelocity(LINEAR_VELOCITY, -LINEAR_VELOCITY/R);
      }
    }
    else{
      if (delta_theta>=0){
        float R = dist/(2*sin(2*M_PI - delta_theta));
        updatecommandVelocity(LINEAR_VELOCITY, -LINEAR_VELOCITY/R);
      }
      else{
        float R = dist/(2*sin(delta_theta + 2*M_PI));
        updatecommandVelocity(LINEAR_VELOCITY, LINEAR_VELOCITY/R);
